<template>
	...
	<div class="flex flex-col items-center">
		...
		<div class="flex flex-col gap-2 w-full items-center">
			...
			<BaseTransition>
				<BaseButton v-if="solved" @click="toggleSaveHighscoreModal"> SAVE HIGHSCORE </BaseButton>
			</BaseTransition>
...
<script setup lang="ts">
	...
	const swapWithKeys = (key: string) => {
		const whiteTileIndex = tiles.value.findIndex((tile) => tile.color === "white");
		switch (key) {
			case "ArrowLeft":
	...
	function swapWithClick(tileId: number) {
		...
		if (openTile !== undefined) {
			const openTileIndex = tiles.value.findIndex((tile) => tile.id === openTile.id);
